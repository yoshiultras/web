...
function clickHandler(event) {
    document.querySelector('.calc-container').addEventListener('click', (event) => {
        ...
        if (event.target.classList.contains('result')) {
            document.querySelector('span').innerText = evaluate(compile(document.querySelector('span').innerText));
        } else if (event.target.classList.contains('clear')) {
            document.querySelector('span').innerText = '';
        }
        else {
            document.querySelector('span').innerText += `${event.target.innerText} `
        }
    })
}
